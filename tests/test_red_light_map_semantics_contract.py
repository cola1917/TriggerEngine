...
def map_context(frames, map_features=None, future_frames=()):
...
class RedLightMapSemanticsContractTests(unittest.TestCase):
    ...
    def test_red_light_crossing_transition_rejects_late_ego_turn(self):
        ...
        before = aligned_frame(0, (agent(1, x=-3.0, y=0.1, vx=5.0),))
        after = aligned_frame(1, (agent(1, x=2.0, y=0.1, vx=5.0),))
        late_turn = aligned_frame(51, (agent(1, x=8.0, y=-6.0, vx=1.0, vy=-4.0, heading=-1.0),))
        ctx = map_context((before, after), future_frames=(late_turn,))

        result = op.evaluate(ctx, after, after.frame.agent_states[0], args)

        self.assertFalse(result.value)
        self.assertGreater(result.metadata["extended_future_heading_change_rad"], 0.45)

    def test_red_light_crossing_transition_keeps_straight_crossing(self):
        from trigger_engine.operators.builtins import register_builtin_operators
        from trigger_engine.operators.registry import OperatorRegistry
        ...
        before = aligned_frame(0, (agent(1, x=-3.0, y=0.1, vx=5.0),))
        after = aligned_frame(1, (agent(1, x=2.0, y=0.1, vx=5.0),))
        future = aligned_frame(51, (agent(1, x=20.0, y=0.1, vx=5.0, heading=0.0),))
        ctx = map_context((before, after), future_frames=(future,))

        self.assertTrue(op.evaluate(ctx, after, after.frame.agent_states[0], args).value)
